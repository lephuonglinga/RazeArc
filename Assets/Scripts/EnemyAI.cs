...
public class EnemyAI : MonoBehaviour, IDamageable
{
    ...
    void HandleIdle()
    {
        // Lu�n s?n s�ng ph�t hi?n player khi idle
    ...
    void HandleWalk()
    {
        // Lu�n s?n s�ng ph�t hi?n player khi patrol
        if (ReactToPlayer()) return;
    ...
    void HandleAttack()
    {
        agent.SetDestination(transform.position); // ??ng y�n
        LookAtPlayer();
        ...
        if (muzzleFlash != null && gunMuzzle != null)
        {
            ...
            targetPos.y += 1f; // Aim v�o th�n ng??i
    ...
    /// Tr? v? true n?u ?� ??i state (?? d?ng x? l� ti?p).
    /// </summary>
    bool ReactToPlayer()
    {
    ...
    void HandleDefeated()
    {
        ...
        foreach (Rigidbody rb in GetComponentsInChildren<Rigidbody>())
        {
            // Log t?t c? collider info c?a bone n�y
